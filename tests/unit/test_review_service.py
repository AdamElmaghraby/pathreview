...
class TestReviewService:
    ...
    # add this method inside the TestReviewService class
    @pytest.mark.asyncio
    async def test_process_review_notifies_user_when_complete(self, mock_db_session):
        """A completed review should notify the user. FAILS until the webhook exists (issue #87)."""
        review_id = uuid4()
        profile_id = uuid4()
        ...
        mock_db_session.execute = AsyncMock(
            side_effect=[first_call, second_call, second_call, second_call]
        )

        # ARRANGE — fake the 4 pipeline steps so we sail through to "complete"
        with (
            patch(
            ...
            patch(
                ...
                new=AsyncMock(return_value={"sections": [], "overall_score": 9.0}),
            ),
            patch(
                "core.services.review_service._run_safety_checks", new=AsyncMock(return_value=True)
            ),
            patch(
                "core.services.review_service.send_review_ready_notification",
                new=AsyncMock(),
                create=True,
            ) as mock_notify,
        ):
        ...
        # Verify the notification was awaited once
